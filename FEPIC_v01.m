...
This code can work with any size of images. However, we strongly advice
using 450x450 pixel image because default parameters are set by assuming
it and because of speed. If you want to use higher resolution images you 
need to experiment with many paramenters. To reseize image to 450x450 pixel
simply uncomment line xxx.

By default, the code assumes RGB images. If you would like to use grayscale
images, simply comment line xxx.
...
other, decrease d_c. If brightness of spots and background are comparable,
increase d_s. If spots are small, decrease A3d. Most of the time you can
keep std3d as it is, but if it gives false positives for a cloudy like
background, decrease it.
%}
d_s = 10; % standard deviation for the decision boundary (Eq.5)
d_c = 5; % offset for the decision boundary (Eq.5)
A3d = 10; % lower limit of amplitude for the Gaussian surface fit (Eq.6)
std3d = 7; % upper limit of standard deviation for the Gaussian surface fit (Eq.6)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
ylabel('Distance');
box on;
saveas(gcf,strcat(file,'_DB.png')); % uncomment to save the plot
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
figure(2);
imshow(RGB);
% imwrite(RGB,strcat(file,'_B.png')); % uncomment to save it
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
figure(3);
imshow(RGB);
imwrite(RGB,strcat(file,'_C.png')); % uncomment to save it
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
if isfile(strcat(file2,'_S_result.csv'))
    writecell(out,strcat(file2,'_S_result.csv'),'WriteMode','append');
else
    writecell(c_names,strcat(file2,'_S_result.csv'));
    writecell(out,strcat(file2,'_S_result.csv'),'WriteMode','append');
end
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
